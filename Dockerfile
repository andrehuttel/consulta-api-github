...
RUN useradd -u 1000 -ms /bin/bash sb-admin

# Make sure files/folders needed by the processes are accessable when they run under the nobody user
RUN chown -R sb-admin.www-data /run && \
  chown -R sb-admin.www-data /var/lib/nginx && \
  chown -R sb-admin.www-data /var/log/nginx
